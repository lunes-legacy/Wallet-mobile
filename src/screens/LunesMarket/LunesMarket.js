...
class LunesMarket extends Component {
    ...
    this.socket.on('m', message => {
      ...
      if (messageType == CCC.STATIC.TYPE.CURRENTAGG) {
        ...
        if (typeof res.PRICE !== 'undefined') {
          ...
          coinPairs[pair].CURRENTPRICE = CCC.convertValueToDisplay(tsym, coinPairs[pair].PRICE);
          coinPairs[pair].DISPLAYPRICE = `1 BTC | ${coinPairs[pair].CURRENTPRICE}`;
  ...
  getBalance() {
    const finalBalance =
      this.props && this.props.balanceData ? this.props.balanceData.confirmed_balance : 0;
    return finalBalance;
  }
  render() {
    return (
