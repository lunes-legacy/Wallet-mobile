  ...
  renderFeesPicker() {
    return this.state.fees.map(fee => {
      if (this.props.fee) {
        ...
        return <Picker.Item label={`${fee.label} - ${valueFee}`} value={valueFee} key={fee.name} />;
      }
      return <Picker.Item label={fee.label} value={fee.value} key={fee.value} />;
    });
  }
        ...
        <View style={styles.container}>
          ...
          <View>
            {this.state.amountToSend > 0 ? (
              <Button rounded block success style={styles.touchable} onPress={() => this.showPIN()}>
