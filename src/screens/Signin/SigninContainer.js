...
import { isValidUser, isValidSignin, isValidSignUp, getItem } from 'app/src/services/UserServices';
        ...
        <ScrollView style={styles.container}>
          ...
          <Content style={styles.contentTabs}>
            <Tabs tabContainerStyle={styles.tabs}>
              <Tab heading={this.props.I18n.t('SIGNIN').toUpperCase()} textStyle={styles.tabText}>
                <LoginForm
                  {...this.props}
                  user={user}
                  email={email}
                ...
                >
                  <Text uppercase={false} style={styles.textChangePassword}>
                    {upperCaseFirstLetterText(this.props.I18n.t('CHANGE_PASSWORD'))}
                  </Text>
                </Button>
              </Tab>
              <Tab heading={this.props.I18n.t('SIGNUP').toUpperCase()} textStyle={styles.tabText}>
                <LoginForm
                  {...this.props}
                  user={user}
                  email={email}
...
const handleConnectionError = status => !status && { messageKey: 'NOT_CONNECTED' };

const mapStateToProps = state => {
  const {
    signin: { user, appState, problem }
  } = state;
  return {
    user,
    appState,
...
export default connect(mapStateToProps, mapDispatchToProps)(HOCContainer(SigninContainer));
