...
export default class Confirmation extends React.Component {
  ...
  renderInputConfirmation() {
    if (!this.state.isWaitingConfirmation) {
      return <Text style={styles.instructions}>{I18N.t('YOU_WILL_RECEIVE_CODE_SMS')}</Text>;
    }
    return null;
  }
  ...
  onSubmitCode() {
    ...
    this.props.confirmCode(finalCodeInput, this.props.verificationId, this.props.user);
  }
  ...
  renderButtonConfirm() {
    const { code1, code2, code3, code4, code5, code6 } = this.state;
    if (this.state.isWaitingConfirmation && code1 && code2 && code3 && code4 && code5 && code6) {
      return (
      ...
      <Container>
        ...
        <View style={styles.viewConfirmSMS}>
          ...
          <Text style={styles.textConfirmSMS}>
            {I18N.t('CONFIRMATION_VIA')} <Text style={styles.textBold}>SMS</Text>
          </Text>
