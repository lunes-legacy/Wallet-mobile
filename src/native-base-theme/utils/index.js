...
export const RenderIconKPIUtil = status => {
  if (status === 'up') {
    return <Entypo name="arrow-long-up" size={10} color={BosonColors.$bosonLightGreen} />;
  } else if (status === 'down') {
    return <Entypo name="arrow-long-down" size={10} color={BosonColors.$bosonDarkYellow} />;
  }
  return <Entypo name="dot-single" size={10} color={BosonColors.$bosonLightGrey} />;
};
