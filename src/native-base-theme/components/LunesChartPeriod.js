  ...
  renderTouchablePeriod(range, translateName) {
    return (
      <View style={styles.columnPeriod}>
        ...
        >
          <Text style={[styles.labelPeriod, this.isSelected(range)]}>{I18N.t(translateName)}</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    return (
      <View style={styles.containerChart}>
        <View style={styles.containerPeriod}>
          {this.renderTouchablePeriod(periodConstant.PERIOD.RANGE_1D, 'ONE_DAY')}
          <View style={styles.columnBorder} />
          {this.renderTouchablePeriod(periodConstant.PERIOD.RANGE_1W, 'ONE_WEEK')}
          <View style={styles.columnBorder} />
          {this.renderTouchablePeriod(periodConstant.PERIOD.RANGE_1M, 'ONE_MONTH')}
          <View style={styles.columnBorder} />
          {this.renderTouchablePeriod(periodConstant.PERIOD.RANGE_1Y, 'ONE_YEAR')}
          <View style={styles.columnBorder} />
          {this.renderTouchablePeriod(periodConstant.PERIOD.RANGE_MAX, 'ALL_TIME')}
        </View>
      </View>
    );
  }
