...
export default class LunesPickerCountry extends React.Component {
  constructor(props) {
    ...
    const countries = [
      {
        ...
        code: 'BR',
        value: '+55',
        flag: 'https://res.cloudinary.com/luneswallet/image/upload/v1516117025/brazil_kxeh78.png'
      },
      {
        label: 'USD',
        code: 'US',
    ...
    this.state = {
      countries,
      countrySelected: I18n.t('CURRENCY_USER') === 'BRL' ? countries[0] : countries[1]
    };
  }
  ...
  onValueChange(value) {
    ...
    const countryFiltered = this.state.countries.filter(country => country.value === value);
    this.setState({
  ...
  renderCountries() {
    return this.state.countries.map(country => (
      <Picker.Item label={country.label} value={country.value} key={country.code} />
    ));
  }
