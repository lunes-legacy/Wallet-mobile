...
export default class LunesChartMain extends React.Component<{}> {
  render() {
        ...
        <G y={min + 6}>
          <Defs>
            <LinearGradient id="gradTop" x1="0" y1="0" x2="0" y2="20">
              <Stop offset="0" stopColor={bosonColor.$bosonLightYellow} stopOpacity="1" />
              <Stop offset="1" stopColor={bosonColor.$bosonDarkYellow} stopOpacity="1" />
            </LinearGradient>
          </Defs>
          <Rect
            height={30}
    ...
    const TooltipBottom = ({ x, y }) => (
      <G x={x(data.length - 1) - 75} key={'tooltipBottom'} onPress={() => alert('clicked')}>
        <G y={max * 2 + 45}>
          <Defs>
            <LinearGradient id="gradBottom" x1="0" y1="0" x2="0" y2="40">
              <Stop offset="0" stopColor={bosonColor.$bosonLightBlue} stopOpacity="1" />
              <Stop offset="1" stopColor={bosonColor.$bosonLightBlue} stopOpacity="1" />
            </LinearGradient>
          </Defs>
          <Rect
            height={30}
    ...
    return (
      <Container>
        <LunesChartPeriod range={this.props.range} onChangeRange={this.props.changeRange} />
        <View style={{ position: 'absolute', width, height: '100%' }}>
          <AreaChart
            ...
            showGrid={false}
            extras={[HorizontalLineTop, HorizontalLineBottom, TooltipTop, TooltipBottom]}
            ...
            renderGradient={({ id }) => (
              <LinearGradient id={id} x1={'0%'} y={'0%'} x2={'0%'} y2={test}>
                <Stop offset={'0%'} stopColor={'rgb(255, 195, 0)'} stopOpacity={0.7} />
                <Stop offset={'100%'} stopColor={'rgb(255, 195, 0)'} stopOpacity={0} />
              </LinearGradient>
            )}
          />
        </View>
