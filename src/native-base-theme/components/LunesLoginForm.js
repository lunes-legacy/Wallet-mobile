...
export default class LunesLoginForm extends React.Component {
  ...
  checkButtonIsDisabled() {
    ...
    } else if (this.props.modeAuth === 'SIGNIN' && this.state.email && this.state.password) {
      return this.renderButtonSubmit();
    }
  ...
  renderDisabledButton() {
    return (
      <View style={styles.btnSubmit}>
        <LunesGradientButton text={upperCaseFirstLetterText(this.getTextButton())} />
      </View>
    );
  }
  ...
  renderButtonSubmit() {
    return (
      <View style={styles.btnSubmit}>
        <Button rounded block success style={{ height: 50 }} onPress={() => this.onSubmit()}>
          <Text uppercase={false}>{upperCaseFirstLetterText(this.getTextButton())}</Text>
        </Button>
      </View>
    );
  }
