...
import { LunesIconSendPayment, LunesIconReceivePayment } from './LunesCustomIcon';
...
export default class LunesQuotation extends React.Component {
  ...
  renderContent() {
    if (!this.props.icon) {
      return (
        <View>
          ...
          <Text style={styles.textQuotationPrice}>{this.props.ticker.BTC.DISPLAYPRICE}</Text>
        </View>
      );
    }
    if (this.props.type === 'success') {
      return (
        <View style={{ alignItems: 'center' }}>
          <LunesIconReceivePayment color={BosonColors.$bosonLightGreen} size={60} />
    ...
    if (this.props.type === 'warning') {
      return (
        <View style={{ alignItems: 'center' }}>
          <LunesIconSendPayment color={BosonColors.$bosonMediumYellow} size={60} />
          <Text style={styles.textPayment}>{this.props.title}</Text>
        </View>
      );
    }
