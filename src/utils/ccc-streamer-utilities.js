  ...
  convertValueToDisplay: function(symbol, value, filterNumberFunctionAngularJS, type, fullNumbers) {
    ...
    if (type === 'short' || type === '8decimals') {
      if (valueAbs > 10000000000) {
        ...
        return (
          prefix + filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers) + ' B'
        );
      }
      if (valueAbs > 10000000) {
        valueAbs = valueAbs / 1000000;
        return (
          prefix + filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers) + ' M'
        );
      }
      if (valueAbs > 10000) {
        valueAbs = valueAbs / 1000;
        return (
          prefix + filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers) + ' K'
        );
      }
      ...
      if (valueAbs >= 1) {
        return (
          prefix + filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnNormalNumbers)
        );
      }
...
CCC.convertValueToDisplay = function(symbol, value, type, fullNumbers) {
  ...
  if (type === 'short') {
    if (valueAbs > 10000000000) {
      valueAbs = valueAbs / 1000000000;
      return (
        prefix + CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers) + ' B'
      );
    }
    if (valueAbs > 10000000) {
      valueAbs = valueAbs / 1000000;
      return (
        prefix + CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers) + ' M'
      );
    }
    if (valueAbs > 10000) {
      valueAbs = valueAbs / 1000;
      return (
        prefix + CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers) + ' K'
      );
    }
    if (valueAbs >= 1) {
      return (
        prefix + CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnNormalNumbers)
      );
    }
    return prefix + (valueSign * valueAbs).toPrecision(decimalsOnSmallNumbers);
  } else {
